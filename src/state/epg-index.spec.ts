...
describe('rowEpgSnapshot', () => {
    it('returns the current title and a progress percent', () => {
        setEpgProgramIndex(
        ...
        expect(snapshot).toEqual({ nowTitle: 'Breakfast', progress: 50 });
    });

    it('returns null inside a gap — a row with nothing on air has nothing to say', () => {
        setEpgProgramIndex(new Map([['a.nl', [program('a.nl', 0, 'Early'), program('a.nl', 5, 'Later')]]]));
        expect(rowEpgSnapshot('a.nl', 3 * HOUR)).toBeNull();
    });
