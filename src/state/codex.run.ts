...
import { countryForLiveToken } from '../epg/countries-lookup';
...
/**
 * Codex export/import — the working half, behind `codex.actions.ts`'s shim.
 *
 * Signing, the WebCrypto identity, the whole `src/codex/` graph and the
 * country registry hang off this file, and none of it is reachable until
 * either a Settings button is pressed or the boot task publishes the author
 * fingerprint. Kept out of the entry chunk for the same reason
 * `subtitle-search.run.ts` is: `check-dist.mjs` bounds what a webOS 6 /
 * Chromium 87 TV parses before first paint, and this was ~4 KiB gzip of it.
 */
