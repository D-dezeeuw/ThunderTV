...
/**
 * The Settings surface for shared Codexes (stone 10) — the working half,
 * behind `codex-library.actions.ts`'s shim.
 *
 ...
 *
 * None of it is reachable before Settings → Codexes is opened, so it is
 * imported on demand rather than parsed at boot on a TV.
 */
...
/** Follow a Codex by URL, read from the uncontrolled input on submit — matching `playlist.actions.ts`, since a half-typed URL is not state worth making reactive. */
export function addFromInput(): void {
    void run(async () => {
...
export function setAuthorTrust(authorId: string | null, trusted: boolean): void {
    void setTrust(authorId, trusted);
}
