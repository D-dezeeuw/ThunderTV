...
import { applyChannelNameStrip, stripCountryPrefix } from './strip-country-prefix.util';

/** Ported from thunder-tv's `strip-country-prefix.util.spec.ts` (Feature 06.1.7) — every case kept, mechanically translated from Jest to Vitest. */
describe('stripCountryPrefix', () => {
    ...
    describe('dash and colon separators (short-tag prefixes only)', () => {
        ...
        it('strips compound quality/provider tags', () => {
            expect(stripCountryPrefix('4K-DE - The Pitt (2025)')).toBe('The Pitt (2025)');
            expect(stripCountryPrefix('AR-SUBS - Fallout')).toBe('Fallout');
            expect(stripCountryPrefix('4K-OSN+ - The Last of Us')).toBe('The Last of Us');
        });
        ...
        it('never treats numeric fragments as tags', () => {
            expect(stripCountryPrefix('1917 - Documentary')).toBe('1917 - Documentary');
        });

        it('keeps bare 4-5 char words before a spaced dash (real titles)', () => {
            expect(stripCountryPrefix('DUNE - Part Two')).toBe('DUNE - Part Two');
            expect(stripCountryPrefix('ALIEN - Covenant')).toBe('ALIEN - Covenant');
        });
        ...
        it('keeps names whose prefix is not a short uppercase tag', () => {
            expect(stripCountryPrefix('Sky - Sports F1')).toBe('Sky - Sports F1');
            expect(stripCountryPrefix('Discovery - Science')).toBe('Discovery - Science');
