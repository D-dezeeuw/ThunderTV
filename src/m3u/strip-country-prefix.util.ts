/**
 * Ported verbatim from thunder-tv's `libs/shared/m3u-utils/src/lib/strip-country-prefix.util.ts`
 * (Feature 06.1.6) — pure string logic, no interface dependencies, so the
 * port is byte-identical apart from this header comment.
 *
...
function findTagSeparator(name: string): SeparatorMatch | null {
    ...
    for (const separator of [...DASH_SEPARATORS, COLON_SEPARATOR]) {
        ...
        const pattern = separator === COLON_SEPARATOR ? COLON_TAG_PATTERN : TAG_PREFIX_PATTERN;
