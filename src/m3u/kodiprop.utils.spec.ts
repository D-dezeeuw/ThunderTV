...
/** Ported from thunder-tv's `kodiprop.utils.spec.ts` (Feature 06.8.6) — every ClearKey format, legacy form, and unsupported-scheme case kept. */
describe('kodiprop.utils', () => {
    describe('extractDrmFromRaw', () => {
        ...
        it('accepts the org.w3.clearkey license type and dashed UUID kids', () => {
            const dashedKid = '9eb4050d-e44b-4802-932e-27d75083e266'.toUpperCase();
            const drm = extractDrmFromRaw(
                rawWith(
        ...
        it('treats a parseable key without license type as ClearKey', () => {
            const drm = extractDrmFromRaw(
                rawWith(`#KODIPROP:inputstream.adaptive.license_key=${KID_HEX}:${KEY_HEX}`),
            );
        ...
        it('ignores unrelated KODIPROP properties', () => {
            expect(
                extractDrmFromRaw(rawWith('#KODIPROP:inputstream.adaptive.manifest_type=mpd')),
            ).toBeUndefined();
        });
    ...
    describe('isClearKeyLicenseType', () => {
        it.each(['clearkey', 'org.w3.clearkey', ' ClearKey '])('accepts %s', (value) => {
            expect(isClearKeyLicenseType(value)).toBe(true);
        });

        it.each(['com.widevine.alpha', 'com.microsoft.playready', ''])('rejects %s', (value) => {
