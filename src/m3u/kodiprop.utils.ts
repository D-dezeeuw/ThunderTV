import type { ChannelDrm, ChannelDrmClearKeys } from './types';

/**
 * `#KODIPROP:` DRM extraction. Ported from thunder-tv's
 * `libs/shared/m3u-utils/src/lib/kodiprop.utils.ts` (Feature 06.1.4/06.8.2)
 * against `ChannelDrm`/`ChannelDrmClearKeys` from `src/core/storage/records.ts`
 * instead of `@iptvnator/shared/interfaces` — otherwise unchanged.
 *
 ...
 */
export function extractDrmFromRaw(raw: string | undefined): ChannelDrm | undefined {
...
function collectKodipropValues(raw: string | undefined): Map<string, string> {
    ...
    for (const line of raw.split(/\r?\n/)) {
        ...
        if (name === LICENSE_TYPE_PROP || name === LICENSE_KEY_PROP || name === DRM_LEGACY_PROP) {
...
function parseClearKeys(value: string): ChannelDrmClearKeys | undefined {
    ...
    return value.startsWith('{') ? parseClearKeysFromJson(value) : parseClearKeysFromPairs(value);
}

function parseClearKeysFromPairs(value: string): ChannelDrmClearKeys | undefined {
...
function parseClearKeysFromJson(value: string): ChannelDrmClearKeys | undefined {
    let parsed: unknown;
    try {
...
function parseW3cClearKeyLicense(entries: unknown[]): ChannelDrmClearKeys | undefined {
    const keys: ChannelDrmClearKeys = {};
    ...
    for (const entry of entries) {
        ...
        const kidHex = normalizeKeyComponent(typeof kid === 'string' ? kid : undefined);
        const keyHex = normalizeKeyComponent(typeof k === 'string' ? k : undefined);
        if (!kidHex || !keyHex) {
            return undefined;
        }
...
function parseClearKeyMap(record: Record<string, unknown>): ChannelDrmClearKeys | undefined {
    const keys: ChannelDrmClearKeys = {};
...
function normalizeKeyComponent(value: string | undefined): string | undefined {
    ...
    return fromBase64 && HEX_128_BIT_PATTERN.test(fromBase64) ? fromBase64 : undefined;
}
...
/**
 * Simplified for ThunderTV (Feature 06.8.5): `atob` exists in both browsers
 * and module workers, so the Node `Buffer` branch is kept only for the
 * Vitest `node`/jsdom test environment (which does expose `atob` as of
 * Node 18+, but the fallback stays as a defensive no-op path since Vitest's
 * jsdom environment is the actual runtime these specs execute under).
 */
