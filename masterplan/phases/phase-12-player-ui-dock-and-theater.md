...
> **Status:** `partial` · tracker: `not-maintained` — The dock, transport, fullscreen (`fullscreen.ts`), audio/subtitle track menus and zap history all ship. **Not built:** theater mode (12.2) — the app is dock-plus-fullscreen, with no list-collapse state; volume control and its persisted key (12.3, 12.4) — playback uses the OS/native volume and no `player.volume` key exists; and the central keymap module (12.9) — key handling lives in per-surface `handle*Keydown` actions instead.
