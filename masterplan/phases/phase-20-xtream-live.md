...
> **Status:** `shipped` · tracker: `not-maintained` — Categories and live streams flow into the same list UI via `live-rows.ts`/`live.actions.ts`, with `xtream-epg-load.ts` for short EPG and `xtream-refresh.ts` for category caching.
