...
> **Status:** `not-started` · tracker: `not-maintained` — `public/manifest.webmanifest` and the icons exist, and nothing else does. There is no service worker anywhere in the repo, so no app-shell caching, no pinned-Spektrum cache, no update flow, no offline boot, no kill-switch and no install prompt. A manifest without a worker is an install prompt that leads to a blank page offline — UPGRADES **U13** owns this.
