...
> **Status:** `shipped` · tracker: `not-maintained` — The settings panel ships with 18 sections, `config-export.ts` handles import/export with credential redaction, and the central strings module carries three locales — since split into lazy per-locale chunks so a user downloads one language, not three.
