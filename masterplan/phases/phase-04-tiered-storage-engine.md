...
> **Status:** `shipped` · tracker: `current` — Probe, three tiers, demotion and chunked writes all still hold — `src/core/storage/README.md` is the live reference.
...
## Post-merge fix and deploy follow-up

- **`src/core/storage/keys.ts` was accidentally written with a literal NUL byte** as the composite-key separator instead of a plain character — git classified the whole file as binary (`Bin 0 -> 1138 bytes` in the merge diff), which would have made every future diff to that file unreviewable. Caught immediately after merge by checking `git ls-files | xargs file` across the whole repo for any other binary-misclassified source file (none found), fixed in a follow-up commit (`90227da`) switching to a plain `|` separator, re-verified full `tsc`/`lint`/`vitest` green (232/232) before pushing.
- Deployed from clean, merged `main` (`npm run deploy`). The first `curl` fetch of the live URL returned the *previous* (Phase 03-sized, 19 903-byte) `index.html` — a CDN propagation delay, not a failed deploy: `git ls-remote origin gh-pages` showed the new commit had landed, and a second fetch ~30s later returned the correct 21 695-byte Phase 04 build (`x-cache: MISS`, `age: 0`), byte-identical to the local build, with `storage-notice`/`dismissStorageNotice`/`settings-storage-tier` all present and the CDN import map unmodified.
