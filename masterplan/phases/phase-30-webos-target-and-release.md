...
> **Status:** `partial` · tracker: `not-maintained` — Packaging works end to end: `npm run build:lg` runs `vite build --mode webos`, the import-map swap, `es-module-shims` injection and `package-webos.mjs`'s `ares-package`. Phase 35 since gave the TV target the input model this phase assumed and 25 never built. The v1.0.0 production-readiness pass (`becff80`) then closed most of the release half: version 1.0.0 is synchronized across web, desktop, webOS and config exports and gated by `npm run lint:csp`/`lint:version`, `RELEASE_CHECKLIST.md` and `webos/PERFORMANCE-BUDGET.md` are written, the TV size budgets are enforced by `check-dist`, and the security audit landed as a strict CSP with precompiled Spektrum bindings. **Not done:** on-device validation on real hardware (30.3–30.6 — launch time, memory, playback, remote navigation, suspend/resume, which `PERFORMANCE-BUDGET.md` itself says a passing build still needs), the landing page, and the `v1.0.0` tag, which does not yet exist in the repository.
