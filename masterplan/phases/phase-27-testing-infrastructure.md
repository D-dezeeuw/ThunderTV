...
> **Status:** `partial` · tracker: `not-maintained` — Vitest conventions, 182 spec files, the storage test matrix, the Spektrum DOM harness (`src/shared/testing/bind-dom.ts`), worker tests with `worker-settle.ts`, the Xtream fixture mocks and the `npm run verify` pre-merge gate all ship. **Not built: Playwright (27.6)** — `@playwright/test` is still not a dependency, and it is what 02.10.9, 07.5.10 and 07.10.7 were each deferred into. Those three boxes cannot close until this one does.
