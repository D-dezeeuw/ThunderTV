...
> **Status:** `shipped` · tracker: `not-maintained` — `src/xtream/` — `client.ts`, `urls.ts`, `errors.ts`, `coerce.ts` — carries the credential model, the action map, single-owner URL construction, the error taxonomy and schema tolerance. `src/xtream/README.md` is the live reference.
