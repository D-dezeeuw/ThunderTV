...
> **Status:** `partial` · tracker: `not-maintained` — Denormalized favorite/recent snapshots, the Starred view, recent plus zap history, and cross-tier survival all ship (`favorites.ts`, `recent.actions.ts`, `collections.ts`). **Not built:** manual favorites ordering (13.7) and stale-favorite orphan reconciliation (13.8) — neither a reorder action nor any orphan-marking code exists.
