...
> **Status:** `superseded` · tracker: `not-maintained` — Superseded by **Phase 31** (matching and mapping) and **Phase 32** (the Guide itself). The guide grid, time axis, shift/back-to-now and program detail ship via `src/state/guide*.ts`; `src/epg/match.ts` owns the strategy ladder and the persisted mapping, keyed on the curated canonical key rather than this phase's tvg-id-first design.
