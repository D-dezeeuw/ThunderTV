...
> **Status:** `not-started` · tracker: `not-maintained` — `src/core/connect/` holds only `source-key.ts`, the dedup key Phase 07 consumes. There is no fragment schema, no boot-time parse, no `history.replaceState` scrub, no `save=0` mode and no link generator. **This is the largest unstarted item on the 1.0 road**, and `src/core/README.md` says so at the point of use.
