...
> **Status:** `partial` · tracker: `not-maintained` — Search shipped, in a different shape than this phase designed. `src/search/` (`fuzzy.ts`, `normalize.ts`) is a ranked fuzzy engine over four scopes — Live, movies, series, everything — driven by `search/setQueryChannels|Movies|Series`; normalization covers the country prefixes, diacritics and quality suffixes 09.3 asked for. The **group-filter dropdown (09.4) became the category rail and accordion** (`list/expandGroup`, `list/showAllChannels`, `list/handleCategoryRailKeydown`) rather than a `<select>`. **Not built:** the `/` focus shortcut (09.7) and single-`<mark>` match highlighting (09.10) — the latter is now an explicit non-goal, recorded in `src/search/README.md`.
