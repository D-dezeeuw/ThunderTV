...
> **Status:** `partial` · tracker: `not-maintained` — Spatial/D-pad navigation landed later, as **Phase 35** (`src/ui/spatial/`), and covers 25.1 and 25.5 in a stronger form than this phase designed — geometry rather than a hand-authored focus ring. `back-navigation.ts` and the per-surface keydown actions cover much of 25.2. **25.8.5's live region now ships**: one always-present `aria-live="polite"` region (`ui.announcement`, written only through `ui.actions.ts`'s `announce()`) carries storage demotions and playback failures to a screen reader. It blanks and re-sets across a tick so a *repeated* message still announces — a live region is silent when its text does not change, which is precisely the case where the same failure happens twice. **Not built:** the keyboard help overlay (25.3); broader ARIA coverage (25.4) remains thin. UPGRADES **U14** owns the remainder.
