...
> **Status:** `shipped` · tracker: `not-maintained` — `src/player/engine.ts` owns the single `<video>` element, `engine-select.ts` owns the URL→engine decision, `player-engine.ts` the interface. Lazy per-engine imports, the mixed-content pre-check and destroy-before-create teardown are all in place. `src/player/README.md` is the live reference.
