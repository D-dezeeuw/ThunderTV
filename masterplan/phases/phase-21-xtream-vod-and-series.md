...
> **Status:** `shipped` · tracker: `not-maintained` — VOD and series catalogs, the poster grid, inline detail, playback, next-episode resolution and resume positions all ship (`vod.*`/`series.*`, `catalog-*.ts`, `grid-metrics.ts`, `player/position.ts`) — plus downloads, which this phase did not anticipate. Persistence took a shape the plan did not specify: catalogs bypass the bulk-table surface entirely. That is AUDIT §4.6 and UPGRADES **U9**, still open. **One known gap:** 21.9.6's `/` search shortcut does not exist — the same missing shortcut recorded under Phase 09, which owns the fix for both.
