...
> **Status:** `partial` · tracker: `not-maintained` — VOD and series catalogs, the poster grid, inline detail, episode playback and resume positions all ship (`vod.*`/`series.*`, `catalog-*.ts`, `grid-metrics.ts`, `player/position.ts`) — plus downloads, which this phase did not anticipate. **Feature 21.6's next-episode affordance now ships** (`src/xtream/next-episode.ts`, `series.nextPrompt`, `series/playNext`): an ended episode offers the next one — across a season boundary and over gap seasons — and nothing plays without the viewer accepting. 21.6.6's `playback.autoAdvance` seam is still deliberately unbuilt; the offer is the whole behaviour today. Landing it surfaced a real defect: `player.active` was not `mapShaped`, so a movie started after an episode inherited the episode's `series` coordinates by deep merge. Persistence took a shape the plan did not specify: catalogs bypass the bulk-table surface entirely. That is AUDIT §4.6 and UPGRADES **U9**, still open. **One known gap:** 21.9.6's `/` search shortcut does not exist — the same missing shortcut recorded under Phase 09, which owns the fix for both.
...
- [x] **21.4.4** Engine selection unchanged — let `selectEngine()` pick by extension exactly as for live (`.m3u8` → hls.js/native, `.ts` → mpegts.js, `mp4`/`mkv`/other → native `<video>`); zero VOD-specific engine code. *(Landed as `attemptChain()`/`isProgressiveFile()` in `src/player/engine-select.ts`. Until it did, every movie was handed to mpegts.js and then hls.js first — the latter reads a whole response body as text looking for `#EXTM3U` — so a film downloaded in the background behind an endless spinner instead of streaming. `attachAndPlay({ live })`, fed from `player.active.kind`, carries the live/VOD distinction the rest of the way to mpegts.js's `isLive`.)*
