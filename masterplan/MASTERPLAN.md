...
*How much to trust these lines:* ten tasks drawn at random from the
untracked phases were then checked against the code. Nine fell where their
phase's status line predicted; one (21.9.6's `/` shortcut) was a real gap
inside a `shipped` phase, and is now named there. The pattern in the misses
is worth knowing — a task is as often **done somewhere else** (17.2.1's tick
lives in `state/epg.ts`, not `epg/tick.ts`) or **moot by redesign** (09.2.2
guards an optimization that was never built) as it is simply undone. Read a
`shipped` line as "the feature works," not as "every task beneath it was
